...
- webhook(): get YOUR incoming-webhook URL. Any program that can make an HTTP request (a script, a cron, an external service) can POST to it — JSON {subject, body, from?, urgent?} or a raw body with ?subject= — and it lands in your inbox as a message from "webhook (source)", waking you if asleep. Give the URL to programs that must notify you of events; treat it as a secret and webhook_rotate() if it leaks. Webhook messages are PROGRAMMATIC: you cannot reply to them — act on them or report to root.
