...
- **Addresses are hierarchical** and encode spawn lineage (`0` = you/root).
- **Every bubble runs in its own folder.** This is load-bearing, not cosmetic:
  parallel `claude` sessions editing shared files would clobber each other. A
  directory per bubble makes the fleet safe to run. (When bubbles must share one
  repo, the folder is backed by a **git worktree** — same convention, later.)
- **The UI is a zoomable tree** that mirrors the address/folder structure.

## 4. Capability model

Capabilities are explicit and gate how far autonomy spreads.
...
- **`send` in MVP 1 is root↔bubble only.** Workers do not know each other yet.
...
- `send` bus, MCP server exposing `send` to bubbles (root↔bubble only).
- Bubble → root pings render as blinking nodes with a subject text box.
...
**Out (v2 — "coming soon"):**
- Autonomous/perpetual spawn (loosening the grant), peer-to-peer `send`,
  introductions, channels (Discord-style invites), SSH runner, git-worktree backing.
...
| `caps` | capability tokens (spawn, budget), grant/check | `addr` |
| `runner` | `Runner` interface + `LocalRunner` (PTY, process mgmt) | — |
| `mcp` | MCP server exposing `send`/`spawn` tools to a session | `bus`, `caps` |
...
**4. MCP tool tests.** The `send`/`spawn` tools are tested directly against the bus
and `caps` (including: a worker without the grant is denied `spawn`; budget
exhaustion is enforced).
